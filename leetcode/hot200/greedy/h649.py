...
假设每一位参议员都足够聪明，会为自己的政党做出最好的策略，你需要预测谁会获胜。输出应该是 "Radiant" 或 "Dire"。
...
输入: "RDD"
输出: "Dire"
解释: 第一轮中,第一个来自 Radiant 阵营的参议员可以使用第一项权利禁止第二个来自 Dire 阵营的参议员的权利。第二个来自 Dire 阵营的参议员会被跳过因为他的权利被禁止了。第三个来自 Dire 阵营的参议员可以使用他的第一项权利禁止第一个来自 Radiant 阵营的参议员的权利。因此在第二轮只剩下第三个来自 Dire 阵营的参议员拥有投票的权利,于是他可以宣布胜利
...
class Solution:
    def predictPartyVictory(self, senate: str) -> str:
        ...
        while rqueue and dqueue:
            ...
            d = dqueue.popleft()
            if r < d:
                rqueue.append(r + len(senate))
            else:
                dqueue.append(d + len(senate))
